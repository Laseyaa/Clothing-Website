...
$search_query = '';
if (isset($_GET['query'])) {
    $search_query = mysqli_real_escape_string($con, trim($_GET['query']));
}
...
if (!empty($search_query)) {
    $total_items_query .= " WHERE `name` LIKE '%$search_query%' OR `description` LIKE '%$search_query%'";
    $select_product_query .= " WHERE `name` LIKE '%$search_query%' OR `description` LIKE '%$search_query%'";
} else {
    $total_items_query .= " WHERE `description` = 'All-Accessories'";
    $select_product_query .= " WHERE `description` = 'All-Accessories'";
}

$total_items_result = mysqli_fetch_assoc(mysqli_query($con, $total_items_query)) or die('Query failed');

$total_items = $total_items_result['total'];
$total_pages = ceil($total_items / $items_per_page);

$select_product_query .= " ORDER BY $order_by LIMIT $offset, $items_per_page";
...
<div class="container-p">
    <div class="product">
        <div class="row row-2">
            <h2><?php echo !empty($search_query) ? 'Search Results' : 'All Accessories'; ?></h2>
            <form method="GET" action="">
                <input type="hidden" name="query" value="<?php echo htmlspecialchars($search_query, ENT_QUOTES, 'UTF-8'); ?>">
    ...
    <?php if ($current_page > 1): ?>
        <a href="?page=<?php echo $current_page - 1; ?>&sort=<?php echo $sort; ?>&query=<?php echo htmlspecialchars($search_query, ENT_QUOTES, 'UTF-8'); ?>" class="page-link">&laquo; Previous</a>
    <?php endif; ?>

    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
        <a href="?page=<?php echo $i; ?>&sort=<?php echo $sort; ?>&query=<?php echo htmlspecialchars($search_query, ENT_QUOTES, 'UTF-8'); ?>" class="page-link <?php echo ($i == $current_page) ? 'active' : ''; ?>"><?php echo $i; ?></a>
    <?php endfor; ?>

    <?php if ($current_page < $total_pages): ?>
        <a href="?page=<?php echo $current_page + 1; ?>&sort=<?php echo $sort; ?>&query=<?php echo htmlspecialchars($search_query, ENT_QUOTES, 'UTF-8'); ?>" class="page-link">Next &raquo;</a>
    <?php endif; ?>
</div>
